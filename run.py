    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if best_acc < max(self.val_acc['epoch']):
            ...
            predict_test = np.argmax(model.predict(
                [X_test_psd, X_test_eog]), axis=1)
            cm = confusion_matrix(y_test_eog, predict_test,
                                  labels=[0, 1, 2, 3, 4])
            print(cm)
            overall_cm[-1] = cm
...
def run_TSSEN(args):
    global fold_string, best_acc, model, file_dir, overall_cm, X_test_psd, X_test_eog, y_test_eog
    ...
    for fold_idx in range(args.from_fold, args.to_fold + 1):
        ...
        X_train_eog, y_train_eog, X_test_eog, y_test_eog = data_loader_eog.load_data()
        ...
        X_train_psd, _, X_test_psd, _ = data_loader_psd.load_data()

        model_name = f'model_fold{fold_idx:02d}_in{num_folds:02d}.h5'
        model = Model.getModelTSSEN(num_classes, seq_len, width, height, time_filters_nums=args.num_time_filters,
        ...
        model.fit([X_train_psd, X_train_eog], y_train_eog, validation_data=([X_test_psd, X_test_eog], y_test_eog),
                  epochs=args.epoch, batch_size=args.batch_size, callbacks=callbacks_list, verbose=2, shuffle=True)

        end_time_fold_i = time.time()

        y_pred = np.argmax(model.predict([X_test_psd, X_test_eog]), axis=1)
        y_true = y_test_eog
...
def main():
    parser = argparse.ArgumentParser(
        description='TS-SEN on MASS-SS3 - K fold')
